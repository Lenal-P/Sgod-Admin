...
const UserList = () => {
  ...
  const fetchDataList = async () => {
    try {
      const response = await AxiosInstance.get(`${adminPathName.listTeacherEndpoint}&page=${paginationModel.page + 1}&limit=${paginationModel.pageSize}&search=${searchQuery}&sort=${sortValue}:${sortType}`);
  ...
  return (
    <Grid container spacing={6.5}>
      ...
      <AddUserDrawer open={addUserOpen} toggle={toggleAddUserDrawer} fetchDataList={fetchDataList} />
    </Grid>
  )
}
...
export const getStaticProps: GetStaticProps = async () => {
  try {
    const res = await AxiosInstance.get(`${adminPathName.listTeacherEndpoint}`);
